...
class FollowSubscription(Base):
    ...
    __table_args__ = (
        UniqueConstraint(
            "chat_group_id", "external_user_id", "anime_id",
    ...
    anime_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("animes.id", ondelete="CASCADE"), nullable=False)
...
class SubscriptionResourceFilter(Base):
    ...
    id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), primary_key=True)
    subscription_id: Mapped[UUID] = mapped_column(PG_UUID(as_uuid=True), ForeignKey("follow_subscriptions.id", ondelete="CASCADE"), nullable=False)
