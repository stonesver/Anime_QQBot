...
- 每日汇总只统计该群存在有效资源订阅且 Mikan 已实际发布的集数；个人/定向资源提醒
  继续即时发送，不受汇总开关影响；
- 默认锚点为北京时间 22:30，最后一条资源后静默 20 分钟，最迟 23:30；当天没有
  实际资源则不发送；
- 启用 `@全体` 后，发送前必须从 NapCat 查询剩余额度；额度为零或状态无法确认时，
  本次直接失败且不重试；
- 周报默认周日 20:00，优先使用周表图片，失败时回退文字。新周报成功发送并设为精华
  后，才会移除上一份周报精华；
- 投票候选限定 3–6 部内部番剧，可选择“本周最佳、下周最期待、本季度最喜欢、群内
  共看”主题。群成员用 `/番剧 投票 [编号]` 查看或改票，用 `/番剧 取消投票` 撤票。
...
- 内容运营功能逐群开启；一个群每天最多一次汇总 `@全体`，周报不使用 `@全体`；
