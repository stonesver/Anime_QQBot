...
def test_admin_api_registers_only_plugin_prefixed_routes() -> None:
    ...
    assert any(route == "/anime_tracking/mention-policy" for route, *_ in context.routes)
    assert any(route == "/anime_tracking/mention-policy/update" for route, *_ in context.routes)
    assert any(route == "/anime_tracking/mention-policy/restore" for route, *_ in context.routes)
