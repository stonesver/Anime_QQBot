    ...
    def from_tool_args(
        ...
        *,
        ...
        if selection is not None and (
            isinstance(selection, bool) or not 1 <= selection <= 20
        ):
...
class ReadonlyAnimeExecutor:
    ...
    async def execute(
        ...
        *,
        ...
        return self._to_result(request.action, reply)

    def _to_result(
        self, action: ReadonlyAnimeAction, reply: Reply
    ) -> ReadonlyToolResult:
        if reply.candidate_items:
            content = "\n".join(
                f"{index}. {item.title}"
